...
def convert_predictions(batch_entity_clf: torch.tensor, b_entity_clf: torch.tensor,
                        batch: dict,
                        input_reader: BaseInputReader, batch_entity_gts: torch.tensor):
    b_entity_types = (b_entity_clf >= 0.5).squeeze(2)
    # get maximum activation (index of predicted entity type)
    # batch_entity_clf[:, :, 0] = 1 - b_entity_types.float()
    batch_entity_types = batch_entity_clf.argmax(dim=-1)
    # batch_entity_types = batch_entity_clf[:, :, 1:].argmax(dim=-1) + 1
    # apply entity sample mask
    # batch_entity_types *= batch['entity_sample_masks'].long()
    batch_entity_types = batch_entity_types * batch['entity_sample_masks'].long() * b_entity_types.long()

    batch_pred_entities = []
    batch_b_pred_probs = []

    for i in range(batch_entity_types.shape[0]):
        # get model predictions for sample
        entity_types = batch_entity_types[i]
        entity_spans = batch['entity_spans'][i]
        entity_clf = batch_entity_clf[i]
        probs = b_entity_clf[i]
        entity_gts = batch_entity_gts[i]

        # convert predicted entities
        sample_pred_entities, sample_b_pred_probs = _convert_pred_entities(
            entity_types, entity_spans, entity_clf, probs, entity_gts, input_reader)

        batch_pred_entities.append(sample_pred_entities)
        batch_b_pred_probs.append(sample_b_pred_probs)

    return batch_pred_entities, batch_b_pred_probs


def _convert_pred_entities(
        entity_types: torch.tensor, entity_spans: torch.tensor,
        entity_scores: torch.tensor, probs: torch.tensor,
        entity_gts: torch.tensor,
    ...
    pred_entity_probs = probs[valid_entity_indices]
    ...
    for i in range(pred_entity_types.shape[0]):
        ...
        prob = pred_entity_probs[i].item()

        converted_pred = (start, end, entity_type, score, prob)
        converted_preds.append(converted_pred)

    entity_types = entity_types.view(-1)
    for i in range(probs.shape[0]):
        label_idx = entity_gts[i].item()
        prob = probs[i].item()
        converted_probs.append([prob, label_idx])
    return converted_preds, converted_probs
...
def store_predictions(documents, pred_entities, b_pred_ent_probs, store_path):
    predictions = []

    for i, doc in enumerate(documents):
        # tokens = doc.tokens
        tokens = []
        for cur_tokens in doc.tokens:
            tokens.extend(cur_tokens)
        sample_pred_entities = pred_entities[i]

        # convert entities
        converted_entities = []
        for ent_idx, entity in enumerate(sample_pred_entities):
            entity_span = entity[:2]
            span_tokens = util.get_span_tokens(tokens, entity_span)
            entity_type = entity[2].identifier
            prob = entity[3]
            converted_entity = dict(type=entity_type, start=span_tokens[0].index, end=span_tokens[-1].index + 1, prob=prob)
    ...
    with open(store_path + '_b_probs', 'w') as probs_file:
        json.dump(b_pred_ent_probs, probs_file)
