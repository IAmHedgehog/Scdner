...
class SpERTTrainer(BaseTrainer):
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            # forward step
            entity_logits, b_entity_probs = model(
            ...
            batch_loss = compute_loss.compute(
                entity_logits=entity_logits, b_entity_probs=b_entity_probs,
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: BaseInputReader,
        ...
        evaluator = Evaluator(dataset, input_reader, self._tokenizer, predictions_path,
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, total=total, desc='Evaluate epoch %s' % epoch):
                ...
                entity_clf, b_entity_clf = model(
                    encodings=batch['encodings'], context_masks=batch['context_masks'],
                    entity_masks=batch['entity_masks'], entity_sizes=batch['entity_sizes'],
                    inference=True, mentions=batch['mentions'])

                # evaluate batch
                evaluator.eval_batch(entity_clf, batch, b_entity_clf, batch['entity_types'])
