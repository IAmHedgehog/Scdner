...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset) -> List[Entity]:
        ...
        for jentity in jentities:
            sent_id = jentity['sent_id']
            entity_type = self._entity_types[jentity['type']]
            start, end = jentity['start'], jentity['end']

            # create entity mention
            tokens = flat_tokens[start: end]

            phrase = " ".join([t.phrase for t in tokens])
            entity = dataset.create_entity(entity_type, tokens, phrase, sent_id)
...
def _parse_tokens(jtokens, dataset, tokenizer):
    ...
    for tokens in jtokens:
        ...
        for token_phrase in tokens:
            ...
            cur_doc_tokens.append(token)
            doc_encoding += token_encoding
            i += 1

        doc_tokens.append(cur_doc_tokens)

    doc_encoding += [tokenizer.convert_tokens_to_ids('[SEP]')]

    if sum(len(tks) for tks in doc_tokens) > 512:
        print('--------->', sum(len(tks) for tks in doc_tokens))
