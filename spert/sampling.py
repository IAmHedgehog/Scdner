...
def create_train_sample(doc, neg_entity_count: int, neg_rel_count: int, max_span_size: int, rel_type_count: int):
    ...
    for e in doc.entities:
        ...
        pos_entity_dict[e.span] = e.sent_id
        ...
        for size in range(1, max_span_size + 1):
            for i in range(0, (token_count - size) + 1):
                ...
                if span not in pos_entity_spans:
                    span_key = tuple(encodings[span[0]: span[1]])
                    if span_key not in neg_entity_span_dict:
                        neg_entity_span_dict[span_key] = []
                    neg_entity_span_dict[span_key].append((span, size))
                    # neg_entity_spans.append(span)
                    # neg_entity_sizes.append(size)

    # sample negative entities
    neg_entity_keys = random.sample(
        list(neg_entity_span_dict.keys()),
        min(len(neg_entity_span_dict), neg_entity_count))
    for key in neg_entity_keys:
        for span, size in neg_entity_span_dict[key]:
            neg_entity_spans.append(span)
            neg_entity_sizes.append(size)

    # print('sampling ------------>', len(neg_entity_span_dict), len(neg_entity_keys), len(neg_entity_spans))
    ...
    mention_dict = {}
    spans = pos_entity_spans + list(neg_entity_spans)
    for idx, span in enumerate(spans):
        key = tuple(encodings[span[0]: span[1]])
        if key not in mention_dict:
            mention_dict[key] = []
        mention_dict[key].append(idx)

    mentions = []
    for key, value in mention_dict.items():
        # if len(value) > 1 and len(key) > 1:
        if len(value) > 1:
            mentions.append(torch.tensor(value, dtype=torch.long))

    # each span in neg_entity_spans is (start, end)
    ...
    for i1, s1 in enumerate(pos_entity_spans):
        for i2, s2 in enumerate(pos_entity_spans):
            ...
            if s1 != s2 and (s1, s2) not in pos_rel_spans and pos_entity_dict[s1] == pos_entity_dict[s2]:
...
def create_eval_sample(doc, max_span_size: int):
    ...
    return dict(encodings=encodings, context_masks=context_masks, entity_masks=entity_masks,
                ...
                entity_sample_masks=entity_sample_masks, mentions=mentions,
