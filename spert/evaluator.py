...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: BaseInputReader, text_encoder: BertTokenizer,
        ...
        self._b_pred_ent_probs = [] # binary prediction probabilities

        self._pseudo_entity_type = EntityType('Entity', 1, 'Entity', 'Entity')  # for span only evaluation

        self._convert_gt(self._dataset.documents)

    def eval_batch(self, batch_entity_clf: torch.tensor, batch: dict, b_entity_clf: torch.tensor,
                   entity_gts: torch.tensor):
        batch_pred_entities, batch_b_pred_probs = prediction.convert_predictions(
            batch_entity_clf, b_entity_clf, batch, self._input_reader,
            batch_entity_gts=entity_gts)
        self._pred_entities.extend(batch_pred_entities)
        self._b_pred_ent_probs.extend(batch_b_pred_probs)
    ...
    def store_predictions(self):
        prediction.store_predictions(
            self._dataset.documents, self._pred_entities,
            self._b_pred_ent_probs, self._predictions_path)
