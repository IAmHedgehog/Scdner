...
class Entity:
    def __init__(self, eid: int, entity_type: EntityType, tokens: List[Token], phrase: str, sent_id: int):
...
class Document:
    def __init__(self, doc_id: int, tokens: List[List[Token]], entities: List[Entity], relations: List[Relation],
                 encoding: List[int]):
    ...
    @property
    def tokens(self):
        return [TokenSpan(tokens) for tokens in self._tokens]
...
class Dataset(TorchDataset):
    ...
    def create_entity(self, entity_type, tokens, phrase, sent_id) -> Entity:
        mention = Entity(self._eid, entity_type, tokens, phrase, sent_id)
